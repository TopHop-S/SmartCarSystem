...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    rcc_osc_init.HSEState = RCC_HSE_ON;                 /* 使能HSE - 外部高速8Mhz */
    rcc_osc_init.PLL.PLLState = RCC_PLL_ON;             /* 打开PLL */
    rcc_osc_init.PLL.PLLSource = RCC_PLLSOURCE_HSE;     /* 选择HSE作为PLL时钟源 */
    rcc_osc_init.PLL.PLLN = plln;                       /* PLL倍频系数 */
    rcc_osc_init.PLL.PLLM = pllm;                       /* PLL和音频PLL分频系数 */
    switch(pllp){                                       /* PLL的p分频系数 - 配置为168Mhz */
    ...
    rcc_osc_init.PLL.PLLQ = pllq;                       /* PLL的q分频系数 - PLL外设时钟配置为48Mhz */
    ...
    /** 同时配置这些时钟 系统时钟、AHB总线时钟、APB1总线时钟、APB2总线时钟 */
